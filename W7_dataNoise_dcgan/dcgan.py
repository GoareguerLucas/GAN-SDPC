...
dataloader, dataset = load_data(depth+"../../cropped/cp/",opt.img_size,opt.batch_size,rand_hflip=False,return_dataset=True)
...
def noised_batch(batch,bag,taux=0.1):
	...
	# Ajout dans l'image
	clear_batch = batch * torch.tensor(mask).float().cuda()
	noised_batch = clear_batch + torch.tensor(noise).float().cuda()
