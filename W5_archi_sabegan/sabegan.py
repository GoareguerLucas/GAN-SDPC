...
class Generation(nn.Module):
    def __init__(self, nc, ngf, nz, imageSize):
        ...
        if(imageSize == 32):
            # 32 x 32
            self.deconv3 = nn.Sequential(nn.Conv2d(ngf, nc, kernel_size=3, stride=1, padding=1),
										 nn.Tanh()
                                         )
        elif(imageSize == 64):
            # 32 x 32
            ...
            # 64 x 64
            self.deconv4 = nn.Sequential(nn.Conv2d(ngf, nc, kernel_size=3, stride=1, padding=1),
										 nn.Tanh()
                                         )
        else:
            # 32 x 32
            ...
            self.deconv5 = nn.Sequential(nn.Conv2d(ngf, nc, kernel_size=3, stride=1, padding=1),
                                         nn.Tanh()
                                         )
        self.ngf = ngf
        self.imageSize = imageSize
...
for epoch in range(1, opt.n_epochs+1):
    ...
    for i, (imgs, _) in enumerate(dataloader):
        ...
        print(
            "[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f] [LR: %f] [Time: %f s] -- M: %f, k: %f"
    ...
    if epoch % opt.sample_interval == 0:
        save_image(gen_imgs.data[:25], "%s/%d.png" %
                   (opt.sample_path, epoch), nrow=5, normalize=True)
