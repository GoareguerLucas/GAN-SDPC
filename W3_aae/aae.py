...
for epoch in range(opt.n_epochs):
	...
	for i, (imgs, _) in enumerate(dataloader):
		...
		valid = Variable(Tensor(imgs.shape[0], 1).fill_(float(np.random.uniform(0.8, 1.0, 1))), requires_grad=False)
		...
		real_loss = adversarial_loss(discriminator(z), valid) #adversarial_loss(discriminator(encoded_imgs.detach()), valid) #
