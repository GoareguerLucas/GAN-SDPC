...
Le loss de G augmente durant l'entrainement sans que l'ont puisse l'expliquer par la théorie.
Pour verrifier qu'il ne sagit pas d'une erreur d'implémentation il faut tester d'autre code pré-existant et qui montrent des résultats concluant, avec leurs propre dataset et si possible en pytorch.
...
__Résultats__ :
  - Pytorch-GAN : Les images générer sont bien des chiffres MNIST convaincan. Les courbes présente les même caractèristique que les notres, notamment le loss de G augmente.
		Time=4h30
  - PGMC : 
		Time=
