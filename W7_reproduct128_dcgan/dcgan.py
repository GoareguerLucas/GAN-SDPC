...
class Generator(nn.Module):
	...
	def forward(self, z):
		if self.verbose:
			...
			out = self.l1(z)	
			out = out.view(out.shape[0], self.max_filters, self.init_size, self.init_size)
			# Dim : (self.max_filters, opt.img_size/16, opt.img_size/16)
			
			out = self.conv1(out)
			# Dim : (self.max_filters/2, opt.img_size/16, opt.img_size/16)
			out = self.conv2(out)
			# Dim : (self.max_filters/4, opt.img_size/8, opt.img_size/8)
			out = self.conv3(out)
			# Dim : (self.max_filters/8, opt.img_size/4, opt.img_size/4)
			out = self.conv4(out)
			# Dim : (self.max_filters/16, opt.img_size/2, opt.img_size/2)
...
class Discriminator(nn.Module):
	...
	def forward(self, img):
		if self.verbose:
			...
			# Dim : (self.max_filters/4, opt.img_size/8, opt.img_size/8)
			out = self.conv4(out)
			# Dim : (self.max_filters/2, opt.img_size/8, opt.img_size/8)
			out = self.conv5(out)
			# Dim : (self.max_filters, opt.img_size/16, opt.img_size/16)
...
# Vecteur fixe pour faire les samples 
