...
for epoch in range(start_epoch,opt.n_epochs+1):
	...
	for i, (imgs, _) in enumerate(dataloader):
		...
		valid_smooth = Variable(Tensor(imgs.shape[0]//2, 1).fill_(float(np.random.uniform(0.9, 1.0, 1))), requires_grad=False)
		valid = Variable(Tensor(imgs.size(0), 1).fill_(1), requires_grad=False)
		fake = Variable(Tensor(imgs.size(0)//2, 1).fill_(0), requires_grad=False)
		...
		noised_img = real_imgs+rand
		d_x = discriminator(noised_img[:imgs.shape[0]//2])
		...
		# Fake batch
		#Discriminator descision
		d_g_z = discriminator(gen_imgs.detach()[:imgs.shape[0]//2])
		# Measure discriminator's ability to classify real from generated samples
